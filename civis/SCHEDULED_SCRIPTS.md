...
- **Note:** Lamair Bryan's original hold (`--organizer "Carlos,Luana,Rommel"`, from before he had an AB account) was lifted 2026-07-29 after his account activated — his entity was connected to campaign 26 and his 141 members assigned that day. A **new temporary hold** (`--organizer "Carlos,Luana,Rommel,Lamair"`) was added the same day: Tiffany's unstaffed-state population contains ~70 duplicate entity pairs from the `insert_organizing_team` double-insert bug (see that step's note) — remove the flag once the campaign-26 duplicate cleanup has run. Prereq for any future organizer: both ends of a connection must be members of campaign 26 — connect via `update_entity_with_tags(26, interact_id, [])`, NOT the AB UI (the UI add was observed not to persist, 2026-07-02).
