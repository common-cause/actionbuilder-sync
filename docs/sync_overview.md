...
**Organizer assignment.** Each Organizing Team member is wired to the C&O organizer who covers their state so organizers can find *their own people* via AB connections. The connection **is** the assignment; the tag just labels it. Both ends must be members of campaign 26 — connect an organizer via `update_entity_with_tags(26, interact_id, [])`, **not** the AB UI (a UI add was observed not to persist, 2026-07-02). Initial load 2026-07-02: 281 connections (Carlos 41, Luana 99, Rommel 141); Lamair Bryan's 141 (CA/HI/MA/MI/NC) followed 2026-07-29 once his AB account activated. Also 2026-07-29: coverage extended beyond the 24 staffed states — all unstaffed states + territories now route to Tiffany Rubio (organizing intern, entity `c941da14`) in the `organizer_state_map` seed; her 71 members were assigned the same day, after the `remove_ot_duplicates` one-shot cleanup removed the 70 duplicate twins the 2026-06-20/21 `insert_organizing_team` double-insert had created in campaign 26 (BQ mirror lag; model since fixed with a sync_log overlay). Total: 564 assignment connections across five organizers. Only members with no zip-derived state remain unassigned. The remaining campaign-26 duplicates (16 launch-window 6/17–18 pairs + 1 legacy 2025 pair) were triaged and removed the same day — losing twins removed from campaign 26 *and* their state campaign (the legacy pair from 26 only), keepers per `dedup_candidates`; campaign 26 now has zero duplicate-email pairs. The `dedup_candidates` wrapper was also repointed at `removed_campaign_entities` (it previously filtered only `remove_from_campaign` rows, so entities removed by the newer cleanup ops were still re-proposed).
...
### Prerequisites
- **The AB tag value must exist in the network catalog AND be activated in every target campaign** before the API can write it. The Person Signup Helper matches `section` + `field` + `name` to existing tags (unless `action_builder:create_tag` is set — which this sync does not use), and activation is per tag **value** per campaign (`campaigns_tags` junction; Customize → Info → Edit Info checkboxes, or the `AssociateTagToCampaign` GraphQL mutation — see `docs/ab_ui_automation.md`). A write to a value that isn't activated in the target campaign **returns 200 and silently drops** — this dropped every Soapboxx Stories write for two months (value created but never activated anywhere, found+fixed 2026-08-07) and all VA/DC campaign-local tags for three months. After wiring a new platform, confirm the first nightly's writes actually landed: `SELECT * FROM actionbuilder_sync.phantom_tag_writes` must stay empty (7-day grace). (See `docs/actionbuilder_person_signup.md`; and `MEMORY.md` → `ab_notes_field_type` for the notes-field gotcha.)
