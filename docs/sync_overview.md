...
**Organizer assignment.** Each Organizing Team member is wired to the C&O organizer who covers their state so organizers can find *their own people* via AB connections. The connection **is** the assignment; the tag just labels it. Both ends must be members of campaign 26 — connect an organizer via `update_entity_with_tags(26, interact_id, [])`, **not** the AB UI (a UI add was observed not to persist, 2026-07-02). Initial load 2026-07-02: 281 connections (Carlos 41, Luana 99, Rommel 141); Lamair Bryan's 141 (CA/HI/MA/MI/NC) followed 2026-07-29 once his AB account activated. Also 2026-07-29: coverage extended beyond the 24 staffed states — all unstaffed states + territories now route to Tiffany Rubio (organizing intern, entity `c941da14`) in the `organizer_state_map` seed. Her initial run is held (temporary `--organizer` filter in `civis/assign_organizers.sh`) until the campaign-26 duplicate cleanup: the 2026-06-20/21 `insert_organizing_team` runs double-inserted 70 people (BQ mirror lag; model since fixed with a sync_log overlay), and those pairs sit exactly in her unstaffed-state population.
