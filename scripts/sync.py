...
Operations:
    ...
    remove_mobilize_externals One-shot: remove other groups' EP volunteers who entered
...
def remove_mobilize_externals(
    ...
    """
    One-shot cleanup: remove entities that represent another coalition group's
    volunteers who entered AB via the Mobilize path and would not be loaded under
    the current anti-poaching rules (Rule A / Rule B in master_load_qualifiers).

    Reads actionbuilder_sync.mobilize_external_removal and calls delete_person for
    each delete_interact_id, using the per-row campaign_interact_id. Same delete
    shape as remove_ep_externals, but with operation='remove_mobilize_external' in
    sync_log so the cleanups can be told apart in the audit trail.
    """
    logger.info('remove_mobilize_externals: fetching rows from mobilize_external_removal...')
    sql = (
        f'SELECT delete_interact_id, delete_first_name, delete_last_name, '
        f'state_abbr, campaign_interact_id, removal_reason '
        f'FROM `{BQ_PROJECT}.{BQ_DATASET}.mobilize_external_removal`'
    )
    if limit:
        sql += f' LIMIT {limit}'
    rows = _query(bq, sql)
    ...
    if not rows:
        logger.info('remove_mobilize_externals: no rows to process')
        return

    logger.info(f'remove_mobilize_externals: {len(rows)} entities to consider')
    n_ok = n_err = n_skip = 0
    ...
    for row in rows:
        ...
        try:
            ...
            n_ok += 1
            if sync_logger:
                sync_logger.log('remove_mobilize_external', entity_id, campaign_id, 'ok')
            if delay:
                time.sleep(delay)
        except Exception as e:
            err_str = str(e)
            if '404' in err_str:
                ...
                n_skip += 1
                if sync_logger:
                    sync_logger.log('remove_mobilize_external', entity_id, campaign_id, '404')
                if delay:
                    time.sleep(delay)
            else:
                logger.error(f'  ERROR deleting {label}: {e}')
                n_err += 1
                if sync_logger:
                    sync_logger.log('remove_mobilize_external', entity_id, campaign_id,
                                    'error', error_detail=err_str[:500])

    logger.info(
        f'remove_mobilize_externals: done. ok={n_ok} err={n_err} '
        f'skipped={n_skip} (includes 404-already-gone)'
    )
...
def remove_suppressed(
    ...
    """
    Remove suppressed people from all their active campaigns.

    Reads actionbuilder_sync.suppression_removal (suppressed_entities x active
    campaign memberships, minus already-removed) and calls delete_person per row.
    Logs operation='remove_suppressed' (listed in removed_campaign_entities, so
    the feed self-clears and every downstream feed drops the removed memberships).

    The entity keeps existing network-level in AB; the suppression_list guards in
    deduplicated_names_to_load / organizing_team_inserts / organizing_team_connects
    prevent the sync from ever re-adding the person. Manage the list with
    scripts/add_suppression.py.
    """
    logger.info('remove_suppressed: fetching rows from suppression_removal...')
    sql = (
        f'SELECT delete_interact_id, delete_first_name, delete_last_name, '
        f'campaign_interact_id, campaign_name, removal_reason '
        f'FROM `{BQ_PROJECT}.{BQ_DATASET}.suppression_removal`'
    )
    if limit:
        sql += f' LIMIT {limit}'
    rows = _query(bq, sql)
    ...
    if not rows:
        logger.info('remove_suppressed: no rows to process')
        return

    logger.info(f'remove_suppressed: {len(rows)} campaign membership(s) to remove')
    n_ok = n_err = n_skip = 0
    ...
    for row in rows:
        ...
        label = f"{entity_id[:8]}... ({name!r}, campaign={row.get('campaign_name')})"
        ...
        if dry_run:
            logger.info(f'  [DRY-RUN] Would remove {label}')
            n_ok += 1
            continue
        ...
        try:
            ...
            n_ok += 1
            if sync_logger:
                sync_logger.log('remove_suppressed', entity_id, campaign_id, 'ok')
            if delay:
                time.sleep(delay)
        except Exception as e:
            err_str = str(e)
            if '404' in err_str:
                ...
                n_skip += 1
                if sync_logger:
                    sync_logger.log('remove_suppressed', entity_id, campaign_id, '404')
                if delay:
                    time.sleep(delay)
            else:
                ...
                n_err += 1
                if sync_logger:
                    sync_logger.log('remove_suppressed', entity_id, campaign_id,
                                    'error', error_detail=err_str[:500])

    logger.info(
        f'remove_suppressed: done. ok={n_ok} err={n_err} '
        f'skipped={n_skip} (includes 404-already-gone)'
    )
...
OPERATIONS = {
    ...
    'remove_suppressed': remove_suppressed,
