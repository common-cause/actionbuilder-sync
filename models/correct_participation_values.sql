...
newmode_by_entity AS (
  -- Aggregate NewMode submissions across all email addresses for each entity
  SELECT
    aee.entity_id,
    SUM(nma.newmode_submission_count) as total_newmode_submissions
  FROM all_entity_emails aee
...
national_top_performers_by_entity AS (
  ...
  SELECT
    aee.entity_id,
    MAX(CASE WHEN natp.top_national_action_taker = TRUE THEN 1 ELSE 0 END) as top_national_action_taker
  FROM all_entity_emails aee
  INNER JOIN {{ ref('action_network_national_top_performers') }} natp
    ON aee.email_normalized = natp.email_normalized
  GROUP BY aee.entity_id
),
