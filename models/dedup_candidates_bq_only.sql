-- dedup_candidates_bq_only: the original BQ-only deduplication logic.
--
-- This is the authoritative source of which entities ARE duplicates, based
-- purely on what BigQuery's replication of actionbuilder_cleaned shows.
--
-- It is referenced by:
--   - dedup_candidates (thin wrapper) — adds sync_log filtering for sync.py
--   - dedup_ambiguous — intentionally uses this version (not the wrapper)
--     so that the human review queue excludes all known-dedup entities,
--     including ones already removed from AB but not yet reflected in BQ.
--
-- To revert to BQ-only mode: delete dedup_candidates.sql and rename this
-- file back to dedup_candidates.sql. Also revert dedup_ambiguous.sql ref.
