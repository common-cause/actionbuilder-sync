...
value_comparisons AS (
  ...
  SELECT 
    ...
    -- Current tag/tagging interact_ids for removal (simplified version for now)
    CAST(NULL AS STRING) as events_6mo_removal_ids,
    CAST(NULL AS STRING) as recent_event_removal_ids,
    CAST(NULL AS STRING) as first_event_removal_ids,
    CAST(NULL AS STRING) as action_network_removal_ids,
    CAST(NULL AS STRING) as state_actions_removal_ids,
    CAST(NULL AS STRING) as top_state_performer_removal_ids,
    CAST(NULL AS STRING) as phone_calls_removal_ids,
...
updates_to_apply AS (
  ...
  SELECT 
    ...
    events_6mo_removal_ids as removal_ids
  ...
  SELECT 
    ...
    recent_event_removal_ids as removal_ids
  FROM value_comparisons
  ...
  SELECT 
    ...
    first_event_removal_ids as removal_ids
  FROM value_comparisons
  ...
  SELECT 
    ...
    action_network_removal_ids as removal_ids
  FROM value_comparisons
  ...
  SELECT 
    ...
    state_actions_removal_ids as removal_ids
  FROM value_comparisons
  ...
  SELECT 
    ...
    top_state_performer_removal_ids as removal_ids
  FROM value_comparisons
  ...
  SELECT 
    ...
    phone_calls_removal_ids as removal_ids
  FROM value_comparisons
...
SELECT 
  ...
  -- Split sync strings into separate columns by field group (only for new values and updates, not clears)
  CASE 
  ...
  -- Split removal IDs into separate columns by field group (only for updates/clears, not new values)
  CASE 
    WHEN field_group = 'Event Attendance History' 
      AND current_value != '' -- Only when there's an existing value to remove
    THEN removal_ids 
    ELSE NULL 
  END as event_participation_history_tag_remove,
  CASE 
    WHEN field_group = 'Event Attendance Summary' 
      AND current_value != '' -- Only when there's an existing value to remove
    THEN removal_ids 
    ELSE NULL 
  END as event_participation_summary_tag_remove,
  CASE 
    WHEN field_group = 'Online Actions Past 6 Months' 
      AND current_value != '' -- Only when there's an existing value to remove
    THEN removal_ids 
    ELSE NULL 
  END as online_actions_past_6_months_tag_remove,
  CASE 
    WHEN field_group = 'State Online Actions' 
      AND current_value != '' -- Only when there's an existing value to remove
    THEN removal_ids 
    ELSE NULL 
