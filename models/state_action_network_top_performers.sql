...
ranked_users AS (
  -- Rank users by state action count
  SELECT 
    *,
    ROW_NUMBER() OVER (ORDER BY state_actions_6_months DESC, user_id) as action_rank
  FROM user_state_action_counts
),

threshold_calculation AS (
  -- Find the threshold for top ~200 performers
  SELECT 
    CASE 
      WHEN COUNT(*) <= 200 THEN MIN(state_actions_6_months)  -- If ≤200 people, include all
      ELSE (
        SELECT state_actions_6_months 
        FROM ranked_users 
        WHERE action_rank = 200
      )
    END as threshold_actions
  FROM ranked_users
),

final_results AS (
  -- Apply threshold and create flags
  SELECT 
    ru.user_id,
    ru.email_normalized,
    ru.residence_state,
    ...
    -- Top performer flag
    CASE 
      WHEN ru.state_actions_6_months >= tc.threshold_actions THEN TRUE
      ELSE FALSE
    ...
    CASE 
      WHEN ru.state_actions_6_months >= tc.threshold_actions 
    ...
    tc.threshold_actions
    
  FROM ranked_users ru
  CROSS JOIN threshold_calculation tc
)
